...
$("#applTypeName").val("<?php echo $Temp[0]->appltypecode.'-'.$Temp[0]->appltypeshort.'-'.$Temp[0]->feerequired; ?>");
...
if($("#sbmt_case").val()=='U')
{
 ...
 $("#applCatName").css('pointer-events', 'none');
                ...
                success: function (json1) {
                  ...
                  for(var i=0;i<=json1.length;i++){
                   ...
                   // console.log(json1[i].nametitle);
                    ...
                    if(json1[i].petcausetitle==null)
                   {

                     $('#populate').prop('checked', false); // Unchecks it

                    }
                    else{
                      var checked='checked';
                      $('input[name^="populate"][value="' + checked + '"').prop('checked', true);

                     }
                  $("#textcausetitle").val(json1[i].petcausetitle);
...
var recpdate = obj.receiptdate;
    ...
    //  row.append('<td><a href="#" data-value="'+obj.receiptSrNo+'" class="receiptClick" >' + count + '</td>');
    row.append('<td class="col-md-2"><a href="#" data-value="" >' + obj.receiptno + '</td>');
                      ...
                      for(var i=0;i<json1.length;i++){
                        $('#isAdvocate').find(':radio[name=isAdvocate][value="'+json1[i].issingleadv+'"]').attr('checked', true);
                         ...
                         // $("#resReltaion >  [value='" + relvalue +"']").attr("selected",true);
                          ...
                          if(json1[i].rescausetitle==null)
                          {

                            $('#populate1').prop('checked', false); // Unchecks it

                           }
                           else{
                             var checked='checked1';
                             $('input[name^="populate1"][value="' + checked + '"').prop('checked', true);

                            }
                            $("#textcausetitle1").val(json1[i].rescausetitle);
  ...
  if(appltype1[1]!='OA')
  {
      var refer = <?php echo json_encode($TempApplTypeRefer) ?>;
             ...
             /* // $("#modal-default").modal('show');
               $("#appl-title").text(appltype1[1]+" Details");
                applDate = refer[i].applicationdate;
                applDate1 = applDate.split('-');
                applRegDate = refer[i].registerdate;
                applRegDate1 = applRegDate.split('-');
                applDisposeDate = refer[i].registerdate;
                applRegDate1 = applDisposeDate.split('-');
                var myString = refer[i].referapplid;
                var newString = myString.substr(2);


                $("#displAppl1").show();
                $("#displAppl2").show();
                $("#modl_applno").val(newString);
                $("#modl_appldate").val(applDate1[2]+'-'+applDate1[1]+'-'+applDate1[0]);
                $("#modl_regdate").val(applRegDate1[2]+'-'+applRegDate1[1]+'-'+applRegDate1[0]);
                $("#modl_subject").val(refer[i].subject);
