...
class CaseFollowUpModel extends Model
{
	...
	public static function getofficenoteDetails($applicationid, $startdate,$enddate)
    {
      if($startdate == '' &&  $enddate == '')
       ...
       // print_r("cdfdfdf");
        $value = DB::select("
       	     (SELECT applicationid,hearingdate as date,'' as officenote,courtdirection FROM public.dailyhearing where applicationid=:applicationid  and courtdirection !=''
			)
			UNION
			(
			SELECT applicationid, officenotedate as date, officenote,''	FROM public.officenote  where applicationid=:applicationid 
			) order by 2",['applicationid' => $applicationid]);
            return $value;
    }else{
       //   print_r("mini");
       $value = DB::select("
                (SELECT applicationid,hearingdate as date,'' as officenote,courtdirection 
            ...
            UNION
            (
            SELECT applicationid, officenotedate as date, officenote,'' 
