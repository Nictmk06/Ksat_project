  ...
  public function getDisposedApplicantExist($application_id,$startno,$applType,$applYear,$endno)
    {
     // $value1 = DB::select("select count(*) as applcount from applicationdisposed where ((applicationsrno between '".$startno."' and '".$endno."') or  (applicationtosrno between '".$startno."' and '".$endno."')) and applicationyear='".$applYear."' and appltypecode='".$applType."'");
       $value1 = DB::select("select * from applicationdisposed where ((applicationsrno between '".$startno."' and '".$endno."') or  (applicationtosrno between '".$startno."' and '".$endno."')) and applicationyear='".$applYear."' and appltypecode='".$applType."'");
        return $value1;
      
    }

     public static function getDisposedApplicationDetails($applicationid,$establishcode)
     {              
        $enteredfrom="";      
       $applicationdisposed= DB::table('applicationdisposed')->where('applicationid', $applicationid)->where('establishcode', $establishcode)->get();
        if (count($applicationdisposed)>0)
         {
           ...
           if($enteredfrom=='Legacy'){  
               ...
               ->join('applicationdisposed', 'applicationdisposed.applicationid', '=', 'application.applicationid')
                ->select('application.*','applicationtype.*','applicationdisposed.disposeddate','applicationdisposed.enteredfrom','referapplid','benchcode')
