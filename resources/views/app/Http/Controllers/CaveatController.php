...
class CaveatController extends Controller
{
	...
	public function search(Request $request){
		...
		//$data['adv_details'] = DB::table('advocate')->select('advocatename')->where('advocatename','LIKE','%'.$application_id.'%')->first();
		//$msg = "This is a simple message.";
		//return response()->json(['success'=>'Data is successfully Added']);
		//return response()->json(['success'=>'Got Simple Ajax Request.','value'=>$name]);
		//$search ='C124/EST-3/2019'
		$appdata = $this->caveatapplicant->getApplicants($application_id);
		$result = DB::table('caveatapplicant')->select('*')->where('caveatid','=',$application_id)->orderBy('applicantsrno', 'asc')->get();
		//dd($result);
		$test = json_encode($appdata);
		//print_r($data['AJAX']);
		//return response()->json($data);
		
		//dd($data);
		//dd($data);
		//echo json_encode($data['adv_details']);
		//return view('caveat.caveat',$data)->with('user',$request->session()->get('userName'));
	...
	public function ressearch(Request $request){
		$application_id = $request->get('application_id');
		$resdata = $this->caveatapplicant->getRespondants($application_id);
		return response()->json([
            'success'=>'Got Simple Ajax Request.',
	...
	public function store(Request $request)
	{
		...
		if($sbmt_case=='A'){	
		$ccode=DB::table('caveat')->max('caveatsrno');
		$ccode++;
