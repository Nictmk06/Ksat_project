        ...
        var establishfullname="<?php echo $establishmentname; ?> "
            ...
            jq(document).ready(function () {
                ...
                jq('#myTable').dataTable({
                    ...
                    dom: 'Bfrtip',
                      buttons: [
                { extend: 'copy',messageTop: 'Receipt issued and not Realized from  '+fromdate+' to '+  todate,title:establishfullname,footer: true,header:true},
                { extend: 'excel', messageTop: 'Receipt issued and not Realized from '+fromdate+' to '+  todate,title: establishfullname, footer: true,header:true },
                { extend: 'csv', messageTop: 'Receipt issued and not Realized from '+fromdate+' to '+  todate,title: establishfullname, footer: true,header:true },
                { extend: 'pdf', messageTop: 'Receipt issued and not Realized from '+fromdate+' to '+  todate,title:establishfullname, footer: true,header:true},
                { extend: 'print', messageTop: 'Receipt issued and not Realized from '+fromdate+' to '+  todate,title: establishfullname, footer: true,header:true}
  ...
  <div class="panel  panel-primary">
    ...
    <table class="table table-bordered table2 display" data-page-length='25' id="myTable" width="100%">
        <thead>
          ...
          <tr>
          <td  class="bg-primary text-center" colspan="8"> <h4> Receipt Issued and not Realized  from <?php echo   date('d/m/Y', strtotime($fromdate)) ?> to <?php echo   date('d/m/Y', strtotime($todate)) ?></h4> </td>
          </tr>
<tr>
<td>Sr.No</td>
<td>Receipt Date</td>
