...
<script type="text/javascript">
    ...
    var statusname="<?php foreach($statusname as $statusname)
       echo $statusname->ccastatusdesc;?>"
      jq(document).ready(function () {
               ...
               jq('#myTable').dataTable({
                ...
                dom: 'Bfrtip',
                  buttons: [
            { extend: 'copy',messageTop: 'List of CCA status for '+statusname +' from '+fromdate+' to '+ todate ,title:establishfullname,footer: true,header:true,pageSize : 'LEGAL',orientation : 'landscape'},
            { extend: 'excel', messageTop: 'List of CCA status for '+statusname +' from '+fromdate+' to '+todate,title: establishfullname, footer: true,header:true,pageSize : 'LEGAL',orientation : 'landscape' },
            { extend: 'csv', messageTop: 'List of CCA status for '+statusname +' from '+fromdate+' to '+todate,title: establishfullname, footer: true,header:true ,pageSize : 'LEGAL',orientation : 'landscape'},
            { extend: 'pdf', messageTop: 'List of CCA status for '+statusname+' from '+fromdate+' to '+todate,title: establishfullname, footer: true,header:true,pageSize : 'LEGAL',orientation : 'landscape'},
            { extend: 'print', messageTop: 'List of CCA status for '+statusname+' from '+fromdate+' to '+ todate,title: establishfullname, footer: true,header:true,pageSize : 'LEGAL',orientation : 'landscape'}
    ...
    <table class="table table-bordered table2 display" data-page-length='25' id="myTable" width="100%">
        <thead>
          ...
          <td  class="bg-primary text-center" colspan="9"> <h3>  <?php echo $establishmentname; ?></h3> </td>
            ...
            <tr>
          <td  class="bg-primary text-center" colspan="9"> <h4>  List of CCA status for <?php  foreach ($statusname as $statusname){
