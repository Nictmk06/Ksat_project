    ...
    var establishfullname="<?php echo $establishmentname; ?> "
      jq(document).ready(function () {
               ...
               jq('#myTable').dataTable({
                ...
                dom: 'Bfrtip',
                  buttons: [
            { extend: 'copy',messageTop: 'List of Applications received with fees from '+fromdate+' to '+  todate ,title:establishfullname,footer: true,header:true},
            { extend: 'excel', messageTop: 'List of Applications received with fees from '+fromdate+' to '+  todate,title: establishfullname, footer: true,header:true },
            { extend: 'csv', messageTop: 'List of Applications received with fees from '+fromdate+' to '+  todate,title: establishfullname, footer: true,header:true },
            { extend: 'pdf', messageTop: 'List of Applications received with fees from '+fromdate+' to '+  todate,title: establishfullname, footer: true,header:true},
            { extend: 'print', messageTop: 'List of Applications received with fees from '+fromdate+' to '+  todate,title: establishfullname, footer: true,header:true}
    ...
    <table class="table table-bordered table2 display" data-page-length='25' id="myTable" width="100%">
        <thead>
          ...
          <td  class="bg-primary text-center" colspan="8"> <h3>  <?php echo $establishmentname; ?></h3> </td>
            ...
            <tr>
          <td  class="bg-primary text-center" colspan="8"> <h4>  List of Application Received with fees from <?php echo   date('d/m/Y', strtotime($fromdate)) ?> to <?php echo   date('d/m/Y', strtotime($todate)) ?></h4> </td>
          </tr>
...
<td>Application Fee</td>
    ...
    <tbody>
      ...
      <?php $sum=0;?>
           ...
           <tr>
            <?php
            $applicationo=$result1->applicationo;
            $registerdate=  $result1->registerdate1;
