        ...
        var establishfullname="<?php echo $establishmentname; ?> "
             jq(document).ready(function () {
                ...
                jq('#myTable').dataTable({
                    ...
                    dom: 'Bfrtip',
                      buttons: [
                { extend: 'copy',messageTop: 'List of scrutinized application  from '+fromdate+' to '+  todate,title: establishfullname,footer: true,header:true},
                { extend: 'excel', messageTop: 'List of scrutinized application from '+fromdate+' to '+  todate,title:establishfullname, footer: true,header:true },
                { extend: 'csv', messageTop: 'List of scrutinized application from '+fromdate+' to '+  todate,title: establishfullname, footer: true,header:true },
                { extend: 'pdf', messageTop: 'List of scrutinized application from '+fromdate+' to '+  todate,title: establishfullname, footer: true,header:true},
                { extend: 'print', messageTop: 'List of scrutinized application from '+fromdate+' to '+  todate,title: establishfullname, footer: true,header:true}
  ...
  <div class="panel  panel-primary">
    ...
    <table class="table table-bordered table2 display"  data-page-length='25' id="myTable" width="100%">
        <thead>
          ...
          <tr>
          <td  class="bg-primary text-center" colspan="8"> <h4>  List of scrutinized application from <?php echo   date('d/m/Y', strtotime($fromdate)) ?> to <?php echo   date('d/m/Y', strtotime($todate)) ?> </h4> </td>
          </tr>
...
<?php $sum=0;?>
           ...
           <tr>
            <?php
            $scrutinydate=$result1->scrutinydate;
            ...
            $acceptreject=$result1->status;
            $reason=$result1->reason;
         ?>
             <td align="center">{{$i++}}</td>
               <td><?php echo date('d/m/Y', strtotime($scrutinydate)); ?></td>
