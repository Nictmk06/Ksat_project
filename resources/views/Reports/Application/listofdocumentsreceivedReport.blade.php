...
<script src="assets/js/search_filter/dataTables.responsive.min.js"></script>
    ...
    <script type="text/javascript">
        ...
        var documentname="<?php
                       foreach ($documentname as $documentname){
                            echo $documentname->documentname;
                          }  ?>"  ;
        ...
        var establishfullname="<?php echo $establishmentname; ?> "
             jq(document).ready(function () {
                ...
                jq('#myTable').dataTable({
                    ...
                    dom: 'Bfrtip',
                      buttons: [
                        { extend: 'copy',messageTop: ' List of Documents received for ' +documentname+ ' type from '+fromdate+' to '+  todate,title: establishfullname,footer: true,header:true,pageSize : 'LEGAL',orientation : 'landscape'},
                        { extend: 'excel', messageTop: ' List of Documents received for '+ documentname+' type from '+fromdate+' to '+  todate,title:establishfullname, footer: true,header:true,pageSize : 'LEGAL',orientation : 'landscape' },
                        { extend: 'csv', messageTop: 'List of Documents received for '+ documentname +' type from '+fromdate+' to '+  todate,title: establishfullname, footer: true,header:true,pageSize : 'LEGAL',orientation : 'landscape'},
                        { extend: 'pdf', messageTop: ' List of Documents received for '+documentname +' type from '+fromdate+' to '+  todate,title: establishfullname, footer: true,header:true,pageSize : 'LEGAL',orientation : 'landscape'},
                        { extend: 'print', messageTop: 'List of Documents received for '+ documentname +'type from '+fromdate+' to '+  todate,title: establishfullname, footer: true,header:true, pageSize : 'LEGAL',orientation : 'landscape'}
  ...
  <div class="panel  panel-primary">
    ...
    <table class="table table-bordered table2 display"  data-page-length='25' id="myTable" width="100%">
        <thead>
          ...
          <tr>
          <td  class="bg-primary text-center" colspan="8"> <h4>  List of Document Received For <?php  foreach ($documentname as $documentname){

                                echo $documentname;
                              }?> Type from  <?php echo   date('d/m/Y', strtotime($fromdate)) ?> to <?php echo   date('d/m/Y', strtotime($todate)) ?> </h4> </td>
          </tr>
...
<td>Status</td>
    ...
    <tbody>
           ...
           <tr>
            ...
            $iano=$result1->iano;
            $ianaturedesc=$result1->ianaturedesc;
            $iafillingdate=$result1->iafillingdate;
            $iaregistrationdate=$result1->iaregistrationdate;
            $statusname=$result1->statusname;
             ...
             <td align="center">{{$i++}}</td>
               ...
               <td><?php echo $iano; ?></td>
               <td><?php echo $ianaturedesc; ?></td>
               <td><?php echo date('d/m/Y', strtotime($iafillingdate));?></td>
               <td><?php echo date('d/m/Y', strtotime($iaregistrationdate));?></td>
