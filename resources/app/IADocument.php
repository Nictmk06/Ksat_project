...
class IADocument extends Model
{
     ...
     protected $fillable = [ 'applicationid','iafillingdate','ianaturecode','iaregistrationdate','hearingdate','orderdate','disposeddate','remark','iastatus','benchcode', 'purposecode', 'courthallno','createdon', 'createdby','iasrno','documenttypecode','iano','iaprayer','partysrno','filledby','appindexref'
    ];
...
//        $value = DB::Table('iadocument')->select('iadocument.applicationid','iadocument.documenttypecode','lsla','iafillingdate','ianaturecode','iadocument.iaprayer','filledby','partysrno','iaregistrationdate','startpage','endpage','iasrno','iano','appindexref')->leftJoin('documenttype','documenttype.documenttypecode','=','iadocument.documenttypecode')->leftJoin('applicationindex','applicationindex.documentno','=','iadocument.appindexref')->where('iadocument.documenttypecode',$doctype)->where('iadocument.iasrno',$IASrrno)->where('applicationindex.applicationid',$applicationid)->get();
        $value = DB::select("select i.applicationid,i.documenttypecode,d.lsla, i.iafillingdate,i.ianaturecode,i.iaprayer, i.filledby, i.partysrno,i.iaregistrationdate,a.startpage,a.endpage,i.iasrno,i.iano,i.appindexref from iadocument i left join documenttype d on d.documenttypecode = i.documenttypecode left join applicationindex a on a.applicationid=i.applicationid and a.documentno = i.appindexref where i.applicationid = '". $applicationid . "' and i.iasrno = ". $IASrrno . " and i.documenttypecode = " . $doctype . "");
        
        return $value;
    }
    public static function getPendingIA($docType,$application_id)
    {
    	$value=DB::Table('iadocument')->select('iano')->where('iadocument.documenttypecode', $docType)->where('iadocument.applicationid', $application_id)->where('iadocument.iastatus', 1)->leftJoin('documenttype', 'documenttype.documenttypecode', '=', 'iadocument.documenttypecode')->leftJoin('ianature', 'ianature.ianaturecode', '=', 'iadocument.ianaturecode')->orderby('iasrno','asc')->get();
			return $value;
    }
  ...
  public static function getHearingDetailsByApplication($applicationid)
    {
        $value = DB::select( "select  * FROM public.dailyhearing where applicationid= '". $applicationid . "' order by hearingdate desc limit 1 ");
        return $value;
    }
    ...
    public static function getBenchJudges($benchtype,$display)
    {
        if($display!='')
        {
           $value=DB::table('bench')->select('benchcode','benchtypename','judgeshortname')->where('benchtypename',$benchtype)->where('display','Y')->orderBy('judgeshortname', 'asc')->get();
            return $value;
        }
        else
        {
            $value=DB::table('bench')->select('benchcode','benchtypename','judgeshortname')->where('benchtypename',$benchtype)->orderBy('judgeshortname', 'asc')->get();
            return $value;
        }
    }
    ...
    {
     /*   $value=DB::table('iadocument')->select('*')->leftJoin('documenttype','documenttype.documenttypecode','=','iadocument.documenttypecode')->leftJoin('status','status.statuscode','=','iadocument.iastatus')->where('hearingdate',$hearingdate)->where('applicationid',$applicationid)->where('benchcode',$benchcode)->orWhere()->get();
            return $value;*/
