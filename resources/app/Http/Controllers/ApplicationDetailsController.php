...
// $data['usermenu'] =$this->module->usermenu($request->session()->get('userName'));
 ...
 //   $data['usermenu'] =$this->module->usermenu($request->session()->get('userName'));
//        $data['usermenumodule'] = $this->module->usermenumodule($request->session()->get('userName'));
    return view('searchapplication',compact('applications', 'results', 'appindexes' ,'applicants','respondents','applreliefs','receipts','hearings','appnum','apptype','applagainst','iaotherdocuments'));  
        }              
    }
