 ...
 public function Circular(Request $request)
  {
	...
	public function index(Request $request)
   {
		...
		return view('Application.FreshApplication1',$data);
   }

   public function saveFreshApplication1(Request $request)
    {
      //return $request;
       $request->validate([
        ...
        $applnStore['actcode']= $request->input('actName');
        $establishcode = $request->session()->get('EstablishCode');
        $estgroupsn = $request->session()->get('estgroupsn');
        $appltypeshort = $arr[1];
		...
		$applicationid =  $appltypeshort.'/'.$applicationno.'/'.$applnStore['applicationyear'];
			 ...
			 $this->UserActivityModel->insertUserActivityDtls($useractivitydtls);
			   DB::commit();
			if( $value == true){
				return redirect()->route('FreshApplication1')->with('success', 'Application No. alloted '.$applicationid);
				}
			else{
				 DB::rollback();
					return redirect()->route('FreshApplication1')->with('error', 'Someting went wrong, Application not saved !! ');
				}
        }catch (\Exception $e) {
                //DB::rollback();
				return $e;
		...
		// catch (\Throwable $e) {
                // DB::rollback();
                // return redirect()->route('FreshApplication1')->with('error', 'Someting wrong, Application not saved !!');
            // }
