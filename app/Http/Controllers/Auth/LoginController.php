    ...
    */
    ...
    public function verifyLogin(Request $request)
    {
       ...
       // $establishcode =  $request->input('establishment');
        ...
        $users = new User();
       $userDet = DB::table('userdetails')->select('*')->where('userid', $username)->get();
		if(count($userDet)> 0)
        {
			 $establishcode =  $userDet[0]->establishcode;;
			 $userAppDet = DB::select("SELECT * FROM userpassword where userid='$username' ");
                         $usercheck=DB::SELECT("SELECT userid from userpassword where userid='$username'");
                       if($usercheck!=null){
                      $usercheck=$usercheck[0]->userid;
                        }
				if (!Hash::check($password, $userAppDet[0]->password)) 
                 {
                 return back()->withErrors(['failure' => $password]);
                 }
         ...
         //  $data['userAppDet'] = DB::select('select * from usercount1'); 
           ...
           $data['establishname'] = $this->case->getEstablishName($establishcode);
			...
			//$single_srno = $data['establishname'][0]->single_srno;
            $groupsn = $data['establishname'][0]->groupsn;
			//return $groupsn;
            ...
            //print_r($userlevel);
			 ...
			 //session()->put('single_srno',$single_srno);
			 session()->put('estgroupsn',$groupsn);
         ...
         session()->put('usersessionid',$value);
           ...
           //print_r($value);
            return view('dashboard.dashboardmain',$data);
			  ...
			  return back()->withErrors(['failure' => ['Invalid Username and Password']]);
		 }
  }
