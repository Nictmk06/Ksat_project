...
public function getJudgementExistupload($application_id,$establishcode,$judgementdate)
    {
      if($judgementdate =="")
     {
      $value1 = DB::select("select count(*) as judgementcount from judgement where  applicationid='".$application_id."'and acceptreject='A'  and establishcode=:establishcode",['establishcode' => $establishcode]);
        return $value1;   
        }else{
         // $value1 = DB::table('judgement')
           // ->where('applicationid', '=', $application_id)
           ...
           // ->where('judgementdate', '=', $judgementdate)
          //  ->count();
         $value1 = DB::select("select count(*) as judgementcount from judgement where  applicationid='".$application_id."' and acceptreject='A' and establishcode=$establishcode and judgementdate=:judgementdate",['judgementdate' => $judgementdate]);
        return $value1; 
        }   
    }

    public function deleteJudgementExist($application_id,$judgementdate)
    {
 
           $value1 = DB::select("delete from judgement where  applicationid='".$application_id."' and  judgementdate='".$judgementdate."'   ");
                  return $value1; 

   }

 
 public static function getAllJudgementByApplNo($applicationid,$establishcode)
    {      
...
public static function getJudgementDtlsByDate($applicationid,$judgementdate)
        { 
