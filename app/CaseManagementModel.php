...
class CaseManagementModel extends Model
{
        ...
        public static function getccadetails($establishcode)
		{
			
				$value = DB::select("select a.ccaapplicationno,a.applicationid,caapplicationdate,challan_ref_no,apply_type,ccastatuscode,b.mobile as mobile,c.email as email
				from copyapplication as a
				left join payment_details as b on (a.ccaapplicationno=b.ccaapplicationno )and (a.receiptno =b.challan_ref_no)
				left join users_online as c on(b.mobile=c.mobile)

		                where a.ccastatuscode=1  and a.documenttype=1 and a.establishcode='$establishcode' and  (a.apply_type=2 or a.apply_type=1) order by email  ");


				return $value;
			}

 public static function getccadetails_offline($establishcode)
      {
      $value = DB::select("SELECT a.ccaapplicationno,a.applicationid,caapplicationdate,challan_ref_no,
        apply_type,ccastatuscode,b.mobile as mobile,c.email as email
      from copyapplication as a
      left join payment_details as b on (a.ccaapplicationno=b.ccaapplicationno )and (a.receiptno =b.challan_ref_no)
      left join users_online as c on(b.mobile=c.mobile)
      where a.ccastatuscode=1  and  a.establishcode='$establishcode' and
       (a.apply_type=2 or a.apply_type=1)
      and requested_mode is null order by email ");
     ...
     public static function getccadetailbyccaID($ccano)
     {
     	 $value = DB::select("select a.ccaapplicationno,a.applicationid,to_char(a.caapplicationdate,'DD/MM/YYYY') as caapplicationdate1,challan_ref_no,apply_type,ccastatuscode,b.mobile as mobile,c.email as email
				from copyapplication as a
				inner join payment_details as b on (a.ccaapplicationno=b.ccaapplicationno )and (a.receiptno =b.challan_ref_no)
				inner join users_online as c on(b.mobile=c.mobile)

				where (a.ccastatuscode=1 and a.apply_type=1 and a.ccaapplicationno='".$ccano."' )");

				return $value;
     }

     public static function Insert_cca_email_log($ccano,$email_sent_by,$msg_content){
         $value= DB::select("insert into cca_email_log(ccaapplicationno,email,email_sent_on,email_sent_by)values('".$ccano."','".$msg_content."',now(),'".$email_sent_by."')");
            return $value;
     }

     public static function updateccastatus($ccano)
     {
       
           $value = DB::select("update copyapplication set ccastatuscode=2 where 
	          ccaapplicationno='".$ccano."' ");

				return $value;

     }
     public static function getfacingsheet($applicationID,$ccaapplicationno)
		{
			
				$value = DB::select("select a.*,b.*,a.ccaapplicationno,c.applicationo,to_char(caapplicationdate,'DD/MM/YYYY') as caapplicationdate1,to_char(a.orderdate,'DD/MM/YYYY') as orderdate1,to_char(trsn_timestamp, 'DD/MM/YYYY HH24:MI:SS') as tran_time,to_char(now(), 'DD/MM/YYYY HH24:MI:SS') as currentdate,
CASE 
...
left join applicationdisposedsummary as c on (a.applicationid=c.applicationid )
left join connecetdappldtls as c1 on a.applicationid=c1.applicationid	
where a.ccastatuscode=1   and a.payment_yn='Y' and  (b.status='PAYMENT_DONE' or b.status is null ) and (a.apply_type=1 or a.apply_type=2) and a.applicationid='".$applicationID."'  and a.ccaapplicationno='".$ccaapplicationno."'  ");
...
public static function getTopApplicantDetails($applicationid)
		...
		{
			$value=db::table('actsection')->select('actsectioncode','actsectionname')->orderBy('actsectioncode', 'asc')->get();
			return $value;
		}
		public static function getApplType()
		{
			$value=db::table('applicationtype')->select('appltypedesc','appltypecode','appltypeshort','appltypedefault','applfees','feerequired','referflag')->orderBy('appltypecode', 'asc')->get();
			return $value;
		}

  //== ===gettting dispose date from applicationdispose table===========
		public static function getDisposedate($applicationid)
		{
			$value=DB::select("select TO_char(disposeddate, 'DD/MM/YYYY') disposeddate from applicationdisposed where applicationid='".$applicationid."' ");
			return $value[0]->disposeddate;
		}
	...
	//==========================================================================
		...
		public function getAllApplicationDetails($application_id,$user,$flag,$establishcode)
		{
			...
			if($flag=='application')
			{
				...
				where a.applicationid='".$application_id."' and a.establishcode= $establishcode
			...
			else if($flag=='iadocument' || $flag=='connected')
			{
				...
				$value = DB::select("select a.appltypecode, a.applicationid,a.applicationdate,a.applicationyear,
...
b.appltypecode=a.appltypecode where applicationid='".$application_id."' and a.establishcode= $establishcode");
			return $value;
			}

		}

public function getAllApplicationDetailsDisposed($application_id,$user,$establishcode)
		{

			$value = DB::select("select a.appltypecode, a.applicationid,a.applicationdate,
						a.registerdate,a.applcategory,a.subject,b.appltypeshort,a.applicationsrno,
						a.applicationtosrno from application as a
						left join applicationtype as b on
						b.appltypecode=a.appltypecode where applicationid='".$application_id."' and a.establishcode= $establishcode
						union
						select a.appltypecode, a.applicationid,a.applicationdate,
						a.registerdate,a.applcategory,a.subject,b.appltypeshort,a.applicationsrno,
						a.applicationtosrno from applicationdisposed as a
						left join applicationtype as b on
						b.appltypecode=a.appltypecode where enteredfrom='Legacy'
						and applicationid='".$application_id."' and a.establishcode= $establishcode");
			return $value;
...
//Mini
	...
	public function getApplicationDetailsCauseList($application_id,$user,$flag,$establishcode)
		{

			if($flag=='Fresh')
			{
				$value = DB::select("select a.scrutinyflag,a.appltypecode, a.applicationid,a.applicationdate,
				...
				c.appltypecode=a.appltypecode where a.applicationid='".$application_id."' and a.lastlistdate is null
				       and a.establishcode= $establishcode");
				return $value;
			}
			else if($flag=='Other')
			{
				...
				a.registerdate,a.applcategory,a.subject,b.appltypeshort,a.applicationsrno,a.applicationtosrno from application as a
				left join applicationtype as b on
				b.appltypecode=a.appltypecode where applicationid='".$application_id."'  and (a.statusid='1' or a.statusid is null or a.statusid='3')
			        and a.establishcode= $establishcode           ");
				return $value;
			}
		...
		public function addApplIndexStore($applDet,$application_id,$new_appl_id,$reviewApplId)
		{
			try {
				...
				$value = DB::transaction(function () use($applDet,$application_id,$new_appl_id,$reviewApplId) {
					...
					DB::table('applagainst')->where('applicationid',$application_id)->delete();
					if(!empty($reviewApplId))
					{
						//DB::table('applagainst')->where('applicationid',$application_id)->delete();
						DB::table('applagainst')->insert(array('referapplid'=>$reviewApplId,'applicationid'=>$new_appl_id));
					}
...
/*			else if($applnStore['isinterimrelief']=='Y' && $IAStore=='')
		...
		public static function getDesignationByDepartment($typeval)
		{
			if($typeval!='')
			{
			   $value=db::table('designation')->select('*')->join('department_designation', 'designation.desigcode', '=', 'department_designation.desigcode')->where('department_designation.departmentcode', '=', $typeval)->orderby('designame', 'asc')->get();
		      return $value;
			}
			else
			{
		...
		{
			$value = DB::table('actsection')->where('actcode',$typeid)->orderBy('actcode', 'asc')->get();
			return $value;
		}
		...
		//mini for ungrouping
public function getGroupAppDtlsBysrno($applicationid,$ungroupstno,$ungroupendno)
		{
			$value = DB::select("select  * from groupno where  applicationid='".$applicationid."' and applicationsrno <= $ungroupstno and
				applicationtosrno >= $ungroupendno");
			//$value = DB::table('groupno')->where('applicationid',$applicationid)
				//	->where('applicationsrno','<=',$ungroupstno)
				//	->where('applicationtosrno',' >= ',$ungroupendno)
				//->get();
			//echo dd($value);
			return $value;
		}

public function getAppDtlsBysrno($applicationid,$ungroupstno,$ungroupendno)
		{
			$value = DB::select("select  * from application where  applicationid='".$applicationid."' and applicationsrno <= $ungroupstno and
				applicationtosrno >= $ungroupendno");
			//$value = DB::table('application')->where('applicationid',$applicationid)
				//	->where('applicationsrno',' = ',$ungroupstno)
				//	->where('applicationtosrno',' = ',$ungroupendno)
				//->get();
			//echo dd($value);
