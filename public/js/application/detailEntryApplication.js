...
//	$("#multiorder").css('pointer-events', 'none');	
	...
	$("#actName").css('pointer-events', 'none');
 ...
 $("#resDeptType").change(function(){
	...
	$("#editSearch").click(function(){
	    ...
	    $.ajax({
				type: 'post',
				dataType:'JSON',
				url: 'getApplicationDetailsFreshApplication',
...
/*	var sbmt_value = $("#sbmt_case").val();
	...
	$('.btnNext').click(function(e) {
		...
		if ($("#" + form).parsley().isValid()) {
			/*if (((startno + appl) - 1) != endno) {
				...
				return false;
			} */  //it only require in fresh application(groupno plural remedy is to be considered)
                       // else {
				$('#noOfAppl').parsley().removeError('noOfAppl');
			//}
	...
	$("#saveApplicant").click(function(e) {
		...
		swal({
            ...
            console.log(json[i].petcausetitle);
