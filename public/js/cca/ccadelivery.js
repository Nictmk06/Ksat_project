...
function getCCADetails(applicationId)
{
	$.ajax({
						...
						{
							var dor = obj.copyreadyon;
							var dor_split = dor.split('-');
							var copyreadyon = dor_split[2]+'-'+dor_split[1]+'-'+dor_split[0];
							
						}
						row.append('<td><a href="#" data-value="'+obj.ccaapplicationno+'|'+copyreadyon+'" class="groupClick" >' +obj.ccaapplicationno+'</a></td>');
						row.append('<td>'+obj.caapplicationdate+'</td>');
						...
						$(".groupClick").click(function()
						{
						  ...
						  var copyreadyon =split[1];
						  
						  $('#deliveryDate').datepicker('setStartDate', copyreadyon);
						})
...
$("#deliverycode").change(function(){
  		...
  		{
  			$(".divDeliveredTo").show();
  			$("#deliveredTo").attr('data-parsley-required', true);	
			}
  		else
  		{
  			$(".divDeliveredTo").hide();
			$("#deliveredTo").attr('data-parsley-required', false);	
