...
/*
 $('.dynamic1').change(function(){
  if($(this).val() != '')
  {
   ...
   //alert(dependent);
      $.ajax({
    url:"{{ route('caveatcontroller.district',[],false) }}",
    method:"POST",
 ...
 $('#depttypecode').change(function(){
		...
		if($(this).val() != '')
		  {
		   ...
		   var value = $(this).val();
		   var select = 'distcode'
		   var dependent = 'talukname';
		   var _token = $('input[name="_token"]').val();
		   //alert(select);
		   //alert(value);
		   //alert(dependent);
			  $.ajax({
			url:"{{ route('caveatcontroller.districtApplicant',[],false) }}",
			method:"POST",
			data:{select:select, value:value, _token:_token, dependent:dependent},
			success:function(result)
			{
				var dependent = 'taluknameApp'
			 $('#'+dependent).html(result);
			}
    ...
    function getBarregDetails(value)
  {
		...
		if($(this).val() != '')
		  {
		   ...
		   //alert(dependent);
			  $.ajax({
			url:"{{ route('caveatcontroller.districtRespondant',[],false) }}",
			method:"POST",
			data:{select:select, value:value, _token:_token, dependent:dependent},
			success:function(result)
			{
				var dependent = 'taluknameRes'
			 $('#'+dependent).html(result);
			}
	...
	/*$('.dynamic4').change(function(){
		if($(this).val() != '')
		  {
		   var select = 'depttypecode';
		   var value = $(this).val();
		   var dependent = 'departmentcode';
		   var _token = $('input[name="_token"]').val();
		   //alert(select);
		   //alert(value);
		   //alert(dependent);
			  $.ajax({
			url:"{{ route('caveatcontroller.fetchdeptres',[],false) }}",
			method:"POST",
			data:{select:select, value:value, _token:_token, dependent:dependent},
			success:function(result)
			{
				var dependent = 'resnameofDept'
			 $('#'+dependent).html(result);
			}
