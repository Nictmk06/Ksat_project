...
function executeQuery() {
	...
	$.ajax({
        ...
        url: "getdisplayboard",
        dataType:"JSON",
        data: {"_token": $('#token').val(),establishcode:1},
              ...
              lengthPassedCasesCH1=lengthPassedCasesCH1+1;
           		$('#ch1p').html(json[i].causelistsrno + ' - '+json[i].applicationo);
					  ...
					  if(json[i].courthallno ==2){
           		 		if(json[i].stage =='A'){
           		 			$('#ch2a').html(json[i].causelistsrno + ' - '+json[i].applicationo);
						}
						/*if(json[i].stage =='N'){
           		 			$('#ch2next').html(json[i].causelistsrno + ' - '+json[i].applicationo);
						}*/
						if(json[i].stage =='P'){
						  passedCasesCH2[lengthPassedCasesCH2]=json[i].causelistsrno + ' - '+json[i].applicationo;
						  lengthPassedCasesCH2=lengthPassedCasesCH2+1;
						 $('#ch2p').html(json[i].causelistsrno + ' - '+json[i].applicationo);
						}
						ch2++;
					}
					  if(json[i].courthallno ==3){
           		 		if(json[i].stage =='A'){
           		 			$('#ch3a').html(json[i].causelistsrno + ' - '+json[i].applicationo);
						}
						/*if(json[i].stage =='N'){
           		 			$('#ch3next').html(json[i].causelistsrno + ' - '+json[i].applicationo);
						}*/
						if(json[i].stage =='P'){
               passedCasesCH3[lengthPassedCasesCH3]=json[i].causelistsrno + ' - '+json[i].applicationo;
               lengthPassedCasesCH3=lengthPassedCasesCH3+1;
           		 $('#ch3p').html(json[i].causelistsrno + ' - '+json[i].applicationo);
