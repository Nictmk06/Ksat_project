$(document).ready(function(){
 ...
 //alert(hearingDate);
    $('#nextHearingDate').datepicker('setStartDate', hearingDate);
	  $('#nextHearingDate').datepicker('setEndDate', null);
   ...
   var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
      ...
      //   $('#mainIdLabel').removeClass('highlight');
		...
		var endSessionVal   = admitPause + '^^' + 'E';
        $('#admitButton').val(admitVal);
        $('#pauseButton').val(pauseVal);
        $('#endSessionButton').val(endSessionVal);
...
$("#endSessionButton").button().click(function(){
    var courthallno = $('#courthallno').val();
	 var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
         $.ajax({
         type : 'POST',
         url : "courthallendsession",
         dataType : "JSON",
         data : {_token: CSRF_TOKEN,courthallno:courthallno},
            success: function(response)
            { 
			alert("Session Ended");
...
$("#admitButton, #pauseButton").button().click(function(){
 ...
 $('#listno').change(function() {
      var listno  = $(this).val();
 //     console.log(listno1);
 //     var arr=listno1.split('-');
   //   var listno=arr[0];
   //   console.log(listno);
   //   var benchcode=arr[1];
    //  console.log(benchcode);          
      $.ajax({
      type : 'post',
      ...
      success: function (json) 
        {
          ...
          $('#applicationId').append('<option value=""> Select Application </option>');
           ...
           var option = '<option value="'+json[i].applicationid+':'+json[i].hearingdate+'">'+json[i].causelistsrno+'-->'+json[i].applicationid+'</option>';
