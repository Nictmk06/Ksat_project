...
.then((willDelete) => {
  if (willDelete) {
   ...
   return false;
	...
	//nic team//$("#start,#endPage,#applStartNo,#applEndNo,#noOfRes,#noOfAppl,#rPincode,#receiptNo,#recpAmount,#pincodeAppl,#respincode2").keypress(function(e) {
		$("#start,#endPage,#applStartNo,#noOfRes,#noOfAppl,#rPincode,#receiptNo,#recpAmount,#pincodeAppl,#respincode2").keypress(function(e) {
	...
	if(establishcode==1)
	{
	$('#receiptDate').datepicker('setEndDate', "02-02-2020");
	}
	if(establishcode==2)
	{
	...
	// $("#applEndNo").focusout(function() { //nic team
...
/*	var sbmt_value = $("#sbmt_case").val();
	...
	$('.btnNext').click(function(e) {
		...
		//var endno = parseInt($("#applEndNo").val()); nic team
		var startno = parseInt($("#applStartNo").val());
		$("#" + form).parsley().validate();
		if ($("#" + form).parsley().isValid()) {
			
			// if (((startno + appl) - 1) != endno) {  /// nic team
			// 	alert(((startno + appl) - 1) != endno);
	    ...
	    // if(reviewApplId == '' && ( applnewtype != 'UA' && applnewtype != 'OA' ) )
		if (applnewtype == 'RP' && type !='' && applnewtype == 'review' )
	     {
	...
	$("#sbmt_relief").click(function() {
			...
			//var endno = parseInt($("#applEndNo").val()); nic team
			var endno = parseInt($("#applStartNo").val()); 
			var startno = parseInt($("#applStartNo").val());
			var appl = parseInt($("#noOfAppl").val());
			var arr = appltypename.split('-');
...
function getReliefTable(response)
    ...
    $("#applTypeName").change(function() {
		...
		var newtype = type.split('-');
		var applnewtype = newtype[1];
		//if (applnewtype != 'OA' && type !='' && applnewtype != 'CRL.RP' ){
